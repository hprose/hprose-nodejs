...
FullDuplexSocketTransporter.prototype.send = function(conn, request, completer, id, env) {
    var self = this;
...
FullDuplexSocketTransporter.prototype.sendAndReceive = function(request, completer, env) {
    ...
    if (conn) {
        ...
        this.send(conn, request, completer, id, env);
    }
        ...
        conn.once('connect', function() {
            conn.connected = true;
            self.send(conn, request, completer, id, env);
        });
    }
    else {
        this.requests.push([request, completer, env]);
    }
};
...
HalfDuplexSocketTransporter.prototype.send = function(conn, request, completer, env) {
    var self = this;
    var timeout = env.timeout;
    if (timeout > 0) {
...
HalfDuplexSocketTransporter.prototype.sendAndReceive = function(request, completer, env) {
    if (this.olduri !== this.client.uri) {
        var pool = this.pool;
        for (var i in pool) {
            pool[i].end();
    ...
    var conn = this.fetch();
    if (conn) {
        this.send(conn, request, completer, env);
    }
    else if (this.size < this.client.maxPoolSize) {
        conn = this.create();
        var self = this;
        conn.once('connect', function() {
            conn.connected = true;
            self.send(conn, request, completer, env);
        });
    }
    else {
        this.requests.push([request, completer, env]);
...
function SocketClient(uri, functions) {
    ...
    function sendAndReceive(request, env) {
        ...
        if (_fullDuplex) {
            fdtrans.sendAndReceive(request, completer, env);
        }
        else {
            hdtrans.sendAndReceive(request, completer, env);
        }
        if (env.oneway) completer.complete();
