...
var HproseProxy = function (setFunction,ns) {
    this.get = function (target, property, receiver) {
        if (ns) {
            property = ns + '_' + property;
        }
        var handler= new HproseProxy(setFunction,property);
        handler.apply = function(target, thisArg, argumentsList) {
            argumentsList.toString();
            return setFunction(thisArg,property).apply(null,argumentsList);
        };
        if(!target.hasOwnProperty(property)) {
            return new Proxy(setFunction, handler);
        } else {
