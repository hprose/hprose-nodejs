...
function responseEnd(ostream, request, response, filter) {
    response.emit('end', filter.outputFilter(ostream.toBuffer(), request));
}

function getCallback(service, functionName, functionArgs, byref, resultMode, async, writer, request, response, filter) {
    return function(result) {
        service.emit('afterInvoke', functionName, functionArgs, byref, result, request);
        var ostream = writer.stream;
        if (resultMode === HproseResultMode.RawWithEndTag) {
            ostream.write(result);
            responseEnd(ostream, request, response, filter);
            return true;
        }
...
function HproseService() {
    ...
    function doInvoke(service, istream, request, response) {
        ...
        do {
            ...
            var callback = getCallback(service, functionName, functionArgs, byref, func.resultMode, func.async, writer, request, response, m_filter);
            if (func === m_functions['*']) {
        ...
        if (!async) {
            ostream.write(HproseTags.TagEnd);
            responseEnd(ostream, request, response, m_filter);
        }
    }
    ...
    this._handle = function(data, request, response) {
        data = m_filter.inputFilter(data, request);
