...
function HproseService() {
    ...
    this.addMissingMethod = function(method, obj, execontext, resultMode, async, simple, useHarmonyMap) {
        this.addMethod(method, obj, '*', execontext, resultMode, async, simple, useHarmonyMap);
    };

    this.addAsyncMissingMethod = function(method, obj, execontext, resultMode, simple, useHarmonyMap) {
        this.addMissingMethod(method, obj, execontext, resultMode, true, simple, useHarmonyMap);
    };
    ...
    this.addFunction = function(func, alias, resultMode, async, simple, useHarmonyMap) {
        ...
        if (typeof(alias) === 'string') {
            ...
            m_functions[aliasName] = {method: func, obj: null, execontext: null, resultMode: resultMode, async: async, simple: simple, useHarmonyMap: useHarmonyMap};
    ...
    this.addMethod = function(method, obj, alias, execontext, resultMode, async, simple, useHarmonyMap) {
        ...
        if (typeof(alias) === 'string') {
            var aliasName = alias.toLowerCase();
            m_functions[aliasName] = {method: method, obj: obj, execontext: execontext, resultMode: resultMode, async: async, simple: simple, useHarmonyMap: useHarmonyMap};
            m_funcNames[aliasName] = alias;
        }
        else {
            throw new HproseException('Argument alias is not a string');
    ...
    this.addAsyncMethod = function(method, obj, alias, execontext, resultMode, simple, useHarmonyMap) {
        this.addMethod(method, obj, alias, execontext, resultMode, true, simple, useHarmonyMap);
    };

    this.addMethods = function(methods, obj, aliases, execontext, resultMode, async, simple, useHarmonyMap) {
        var count = methods.length;
        var i;
        if (aliases === undefined || aliases === null) {
            for (i = 0; i < count; i++) {
                this.addMethod(methods[i], obj, null, execontext, resultMode, async, simple, useHarmonyMap);
            }
        }
        else {
            if (count !== aliases.length) {
            ...
            for (i = 0; i < count; i++) {
                this.addMethod(methods[i], obj, aliases[i], execontext, resultMode, async, simple, useHarmonyMap);
            }
        }
    };

    this.addAsyncMethods = function(methods, obj, aliases, execontext, resultMode, simple, useHarmonyMap) {
        this.addMethods(methods, obj, aliases, execontext, resultMode, true, simple, useHarmonyMap);
    };

    this.addInstanceMethods = function(obj, aliasPrefix, execontext, resultMode, async, simple, useHarmonyMap) {
        var alias;
        for (var name in obj) {
            alias = (aliasPrefix ? aliasPrefix + '_' + name : name);
            if (typeof(obj[name]) === 'function') {
                this.addMethod(obj[name], obj, alias, execontext, resultMode, async, simple, useHarmonyMap);
            }
        }
    };

    this.addAsyncInstanceMethods = function(obj, aliasPrefix, execontext, resultMode, async, simple, useHarmonyMap) {
        this.addInstanceMethods(obj, aliasPrefix, execontext, resultMode, true, simple, useHarmonyMap);
    };
