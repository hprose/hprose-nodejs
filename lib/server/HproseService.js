...
function HproseService() {
    ...
    this.addMissingFunction = function(func, resultMode, async, simple, useHarmonyMap) {
        this.addFunction(func, '*', resultMode, async, simple, useHarmonyMap);
    };

    this.addAsyncMissingFunction = function(func, resultMode, simple, useHarmonyMap) {
        this.addMissingFunction(func, resultMode, true, simple, useHarmonyMap);
    };

    this.addMissingMethod = function(method, obj, context, resultMode, async, simple, useHarmonyMap) {
        this.addMethod(method, obj, '*', context, resultMode, async, simple, useHarmonyMap);
    };

    this.addAsyncMissingMethod = function(method, obj, context, resultMode, simple, useHarmonyMap) {
        this.addMissingMethod(method, obj, context, resultMode, true, simple, useHarmonyMap);
    };

    this.addFunction = function(func, alias, resultMode, async, simple, useHarmonyMap) {
        ...
        if (typeof(alias) === 'string') {
            ...
            m_functions[aliasName] = {method: func, obj: null, context: null, resultMode: resultMode, async: async, simple: simple, useHarmonyMap: useHarmonyMap};
    ...
    this.addAsyncFunction = function(func, alias, resultMode, simple, useHarmonyMap) {
        this.addFunction(func, alias, resultMode, true, simple, useHarmonyMap);
    };

    this.addFunctions = function(functions, aliases, resultMode, async, simple, useHarmonyMap) {
        var count = functions.length;
        var i;
        if (aliases === undefined || aliases === null) {
            for (i = 0; i < count; i++) this.addFunction(functions[i], null, resultMode, async, simple, useHarmonyMap);
        }
        else {
            if (count !== aliases.length) {
                throw new HproseException('The count of functions is not matched with aliases');
            }
            for (i = 0; i < count; i++) this.addFunction(functions[i], aliases[i], resultMode, async, simple, useHarmonyMap);
        }
    };

    this.addAsyncFunctions = function(functions, aliases, resultMode, simple, useHarmonyMap) {
        this.addFunctions(functions, aliases, resultMode, true, simple, useHarmonyMap);
    };

    this.addMethod = function(method, obj, alias, context, resultMode, async, simple, useHarmonyMap) {
        if (obj === undefined || obj === null) {
            this.addFunction(method, alias, resultMode, async, simple, useHarmonyMap);
            return;
        }
        ...
        if (typeof(alias) === 'string') {
            var aliasName = alias.toLowerCase();
            m_functions[aliasName] = {method: method, obj: obj, context: context, resultMode: resultMode, async: async, simple: simple, useHarmonyMap: useHarmonyMap};
            m_funcNames[aliasName] = alias;
        }
        else {
            throw new HproseException('Argument alias is not a string');
    ...
    this.addAsyncMethod = function(method, obj, alias, context, resultMode, simple, useHarmonyMap) {
        this.addMethod(method, obj, alias, context, resultMode, true, simple, useHarmonyMap);
    };

    this.addMethods = function(methods, obj, aliases, context, resultMode, async, simple, useHarmonyMap) {
        var count = methods.length;
        var i;
        if (aliases === undefined || aliases === null) {
            for (i = 0; i < count; i++) {
                this.addMethod(methods[i], obj, null, context, resultMode, async, simple, useHarmonyMap);
            }
        }
        else {
            if (count !== aliases.length) {
            ...
            for (i = 0; i < count; i++) {
                this.addMethod(methods[i], obj, aliases[i], context, resultMode, async, simple, useHarmonyMap);
            }
        }
    };

    this.addAsyncMethods = function(methods, obj, aliases, context, resultMode, simple, useHarmonyMap) {
        this.addMethods(methods, obj, aliases, context, resultMode, true, simple, useHarmonyMap);
    };

    this.addInstanceMethods = function(obj, aliasPrefix, context, resultMode, async, simple, useHarmonyMap) {
        var alias;
        for (var name in obj) {
            alias = (aliasPrefix ? aliasPrefix + '_' + name : name);
            if (typeof(obj[name]) === 'function') {
                this.addMethod(obj[name], obj, alias, context, resultMode, async, simple, useHarmonyMap);
            }
        }
    };

    this.addAsyncInstanceMethods = function(obj, aliasPrefix, context, resultMode, async, simple, useHarmonyMap) {
        this.addInstanceMethods(obj, aliasPrefix, context, resultMode, true, simple, useHarmonyMap);
    };
