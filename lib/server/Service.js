...
function Service() {
    ...
    function beforeInvoke(call, name, args, byref, context, output) {
        ...
        if (_onBeforeInvoke !== null) {
            ...
            if (Future.isPromise(value)) {
                return value.then(function(e) {
                    ...
                    var result = invoke(call, name, args, byref, context, output);
                    if (result === null) {
        ...
        return invoke(call, name, args, byref, context, output);
    }

    function invoke(call, name, args, byref, context, output) {
        if (call === _calls['*']) {
            args = [name, args];
        }
        var result;
        ...
        if (Future.isPromise(result)) {
            return result.then(function(result) {
                ...
                return afterInvoke(call, name, args, byref, context, result, new BytesIO(), true);
            },
            function(e) {
                return sendError(e, context);
            });
        }
        return afterInvoke(call, name, args, byref, context, result, output, false);
    }

    function afterInvoke(call, name, args, byref, context, result, output, async) {
        ...
        if (_onAfterInvoke !== null) {
            ...
            if (Future.isPromise(value)) {
                return value.then(function(e) {
                        if (util.isError(e)) throw e;
                        result = doOutput(call, args, byref, context, result, output, async);
                        if (result === null) {
                            output.writeByte(Tags.TagEnd);
                            result = outputFilter(output.bytes, context);
                        }
        ...
        return doOutput(call, args, byref, context, result, output, async);
    }

    function doOutput(call, args, byref, context, result, output, async) {
        var mode = call.mode;
    ...
    function doInvoke(input, context) {
        ...
        do {
            ...
            var result = beforeInvoke(call, name, args, byref, context, output, async);
            if (result !== null) {
                return result;
            }
