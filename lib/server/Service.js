...
function Service() {
    ...
    function setBeforeInvoke(value) {
        if (value === null || typeof value === 'function') {
            ...
            throw new Exception("onBeforeInvoke must be a function or null.")
        }
    }
    ...
    function setAfterInvoke(value) {
        if (value === null || typeof value === 'function') {
            ...
            throw new Exception("onAfterInvoke must be a function or null.")
        }
    }
    ...
    function setSendError(value) {
        if (value === null || typeof value === 'function') {
            ...
            throw new Exception("onSendError must be a function or null.")
        }
    }
    ...
    function sendError(error, context) {
        ...
        stream.writeString(_debug ? error.message : error.stack);
    ...
    function afterInvoke(name, args, byref, mode, simple, context, result, output, async) {
        ...
        if (_onAfterInvoke !== null) {
            var future = _onAfterInvoke(name, args, byref, result, context);
            if (Completer.isFuture(future)) {
                var completer = new Completer();
                future.then(function(e) {
                    if (e instanceof Error) {
                        completer.completeError(sendError(e, context));
                    }
                    else {
    ...
    function invoke(call, name, args, byref, mode, simple, context, output, async) {
        ...
        if (Completer.isFuture(result)) {
            ...
            result.then(callback.handler).catchError(callback.errorHandler);
            return completer.future;
        }
    ...
    function doInvoke(input, context) {
        ...
        do {
            ...
            if (_onBeforeInvoke !== null) {
                var future = _onBeforeInvoke(name, args, byref, context);
                if (Completer.isFuture(future)) {
                    var completer = new Completer();
                    future.then(function(e) {
                        if (e instanceof Error) {
    ...
    function addFunction(func, alias, mode, simple, useHarmonyMap, async) {
        if (typeof(func) !== 'function') {
            throw new Exception('Argument func must be a function');
        }
        if (mode === undefined) {
        ...
        if (typeof(alias) === 'string') {
            ...
            throw new Exception('Argument alias must be a string');
        }
    }
    ...
    function addMethod(method, obj, alias, exec, mode, simple, useHarmonyMap, async) {
        ...
        if (typeof(method) !== 'function' &&
            typeof(obj[method]) !== 'function') {
                throw new Exception('method or obj[method] must be a function');
        }
        if (exec === undefined) {
            exec = obj;
        }
