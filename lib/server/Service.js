...
function callService(call, args) {
    if (call.oneway) {
        setImmediate(function() {
            call.method.apply(call.thisArg, args);
        });
        if (call.async) {
...
function Service() {
   ...
   function setPassContext(value) {
    ...
    function addMethod(method, obj, alias, thisArg, options) {
    ...
    function addAsyncMethod(method, obj, alias, thisArg, options) {
        options = options || {};
        options.async = true;
        addMethod(method, obj, alias, thisArg, options);
    }

    function addMissingMethod(method, obj, thisArg, options) {
        addMethod(method, obj, '*', thisArg, options);
    }

    function addAsyncMissingMethod(method, obj, thisArg, options) {
        options = options || {};
        options.async = true;
        addMissingMethod(method, obj, thisArg, options);
    }

    function addMethods(methods, obj, aliases, thisArg, options) {
        var i;
        if (aliases === undefined || aliases === null || aliases === '') {
            for (i in methods) {
                addMethod(methods[i], obj, null, thisArg, options);
            }
        }
        else {
            ...
            for (i in methods) {
                addMethod(methods[i], obj, aliases[i], thisArg, options);
            }
        }
    }

    function addAsyncMethods(methods, obj, aliases, thisArg, options) {
        options = options || {};
        options.async = true;
        addMethods(methods, obj, aliases, thisArg, options);
    }

    function addInstanceMethods(obj, aliasPrefix, thisArg, options) {
        var alias;
        for (var name in obj) {
            alias = (aliasPrefix ? aliasPrefix + '_' + name : name);
            if (typeof(obj[name]) === 'function') {
                addMethod(obj[name], obj, alias, thisArg, options);
            }
        }
    }

    function addAsyncInstanceMethods(obj, aliasPrefix, thisArg, options) {
        options = options || {};
        options.async = true;
        addInstanceMethods(obj, aliasPrefix, thisArg, options);
    }
